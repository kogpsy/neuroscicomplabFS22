...
Bias (Vorwissen) wurde durch einen Hinweisreiz angezeigt, in Form eines Pfeils oder eines neutralen Stimulus. Der Pfeil zeigte die wahrscheinlichere Bewegungsrichtung an. Vor und nach dem Cue wurde ein Fixationskreuy gezeigt. Alle weiteren Parameter können Sie dem Paper entnehmen [@mulderBiasBrainDiffusion2012a]. 
...
Wenn man die default-Einstellungen nicht ändert, speichert PsychoPy automatisch die Daten einem trial-by-trial CSV File. Dieses CSV File erhält einen Namen, der sich aus der Versuchspersonen-ID, dem Namen des Experiments, und dem aktuellen Datum inkl. Uhrzeit zusammensetzt. So ist es möglich, mit derselben ID beliebig oft das Experiment zu wiederholen. Die CSV Files werden in einem Ordner mit dem Name `data` abgelegt.
...
Zur Umrechnung zwischen cm und _degrees of visual angle_ finden unter diesem 👉 [Link](https://www.sr-research.com/eye-tracking-blog/background/visual-angle/) mehr Information.
