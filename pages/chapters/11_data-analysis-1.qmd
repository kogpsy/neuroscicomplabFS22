...
1) We have assumed that the data are conditionally normally distributed, given the means, with the same variance.
...
over the possible parameter values. To do this, we need to mupltiply the prior probability of each parameter value by the likelihood of the data, i.e. by the probability of observing the data given that parameter value. This is an application of Bayes theorem:
...
1) Represent your prior belief by a probability distribution over the possible parameter values. This is a principled way of daling with uncertainty.
...
:::{.callout-note}
A family of probability distributions that are suitable for parameters that lie oîn the interval $[0,1]$ is the [Beta distribution](https://en.wikipedia.org/wiki/Beta_distribution). This distribution has $2$ parameters  $\alpha$ und $\beta$, 
which can be interpreted as the prior number of successes and the number of failures, respectively. The number of trials is therefore $\alpha + \beta$. \@ref(fig:betadists) shows a number of possible Beta distributions for various setting s of $\alpha$ and $\beta$.
...
We can now plot the resulting normalised posterior distribution. 
```{r}
plot(theta_grid, posterior, type = "l", yaxt = 'n', ylab = 'Probability', 
        main = "Posterior", cex.lab = 1.5, cex.main = 3)
```

You can verify that it is normalized by checking that the sum of the probabilities is 1.
...
TO show how this works, we can draw a thousand samples from the posterior distribution.
