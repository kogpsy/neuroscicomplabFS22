...
compute_posterior = function(likelihood, prior){
  likelihood <- likelihood * (length(likelihood) - 1)
...
```{r}
n_points <- 101
theta_grid <- seq(0, 1, length =  n_points)
...
```{r}
prior <- dbeta(theta_grid, shape1 = 1, shape2 = 1)
posterior <- dbeta(theta_grid, shape1 = 1 + wins, shape2 = 1 + (games - wins))
```

```{r}
bf01 <- dbeta(0.5, shape1 = 1, shape2 = 1)/dbeta(0.5, shape1 = 1 + wins, shape2 = 1 + (games - wins))
```
...
```
```{r}
(bf <- d$prior[51]/d$posterior[51])
```


```{r}
plot(theta_grid, posterior, "l", col = "purple")
lines(theta_grid, prior, col = "blue")
```
...
where $\mathcal{M}$ refers to a specific model. The marginal likelihood $p(y | \mathcal{M})$ now gives the probability of the data, averaged over all possible parameter values under model $\mathcal{M}$.
