...
We first calculate the probability of a hit, `pH`, and a false alarm, `pFA`, as the are under the curve to the right of the criterion, under both signal and noise distributions. 
...
Once we have the number of hits and false alarms, we can compute the number of misses and correct rejections, given that we know how many trials were perfomred in each condition.
...
:::exercise
How can make the ideal observer achieve an (almost) perfect performance?
:::
...
:::exercise
Why is seemingly difficult to recover theses parameters?
:::
